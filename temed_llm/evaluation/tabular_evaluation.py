    ...
    for key in dict1.keys():
        if dict1.get(key) == dict2.get(key):
            ...
            if dict2.get(key) is None:
                none_count += 1
                medical_report_count += count_value_in_medical_report(
                    dict1.get(key), medical_report
                )
                comparison_result[key] = (
                    True if str(dict1.get(key)) not in medical_report else False
                )
...
def create_comparison_columns(
    ...
    for _, row in df.iterrows():
        comparison, none_count, medical_report_count = compare_dicts(
            row[original_json_column_name],
            row[results_column_name],
            row[medical_report_column_name],
        )
...
def get_statistics(df: pd.DataFrame, dataset_name):
    total_rows = len(df)
    valid_rows = len(df[df["validation_result"] is True])
    invalid_rows = len(df[df["validation_result"] is False])
