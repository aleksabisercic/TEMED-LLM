...
class HeartData(BaseModel):
    ...
    exercise_angina: Optional[str] = Field(
        description="Exercise-induced angina [Y,N] where Y: Yes, N: No"
    )
    oldpeak: Optional[Union[float, int]] = Field(
    ...
    def validate_chest_pain_type(cls, v):
        ...
        if v not in allowed_values:
            raise ValueError(
                f"Invalid value for ChestPainType, allowed values are {allowed_values}"
            )
        return v
...
class MentalHelthData(BaseModel):
    ...
    choose_your_gender: Optional[str] = Field(
        None, description="Gender of the student [Female, Male]"
    )
    age: Optional[float] = Field(None, description="Age of the student")
    what_is_your_course: Optional[str] = Field(
        None, description="Course the student is enrolled in"
    )
    your_current_year_of_study: Optional[str] = Field(
        None, description="Current year of study [year 1, year 2, year 3, year 4]"
    )
    what_is_your_cgpa: Optional[str] = Field(
        None,
        description="Current CGPA range of the student [0 - 1.99, 2.00 - 2.49, 2.50 - 2.99, 3.00 - 3.49, 3.50 - 4.00]",
    )
    marital_status: Optional[str] = Field(
        None, description="Marital status of the student [yes, no]"
    )
    do_you_have_depression: Optional[str] = Field(
        None, description="Indicates if the student has depression [yes, no]"
    )
    do_you_have_anxiety: Optional[str] = Field(
        None, description="Indicates if the student has anxiety [yes, no]"
    )
    do_you_have_panic_attack: Optional[str] = Field(
        None, description="Indicates if the student has panic attacks [yes, no]"
    )
    did_you_seek_any_specialist_for_treatment: Optional[str] = Field(
        None,
...
class TreatmentData(BaseModel):
    ...
    haemoglobins: Optional[float] = Field(
        description="Patient laboratory test result of haemoglobins"
    )
    erythrocyte: Optional[float] = Field(
    ...
    thrombocyte: Optional[float] = Field(
...
class StrokeData(BaseModel):
    ...
    Residence_type: str = Field(
        None, description="Residence type of the patient: 'Rural' or 'Urban'"
    )
    ...
    def validate_smoking_status(cls, v):
        ...
        if v not in allowed_values:
            raise ValueError(
                f"Invalid smoking status, allowed values are {', '.join(allowed_values)}"
            )
        return v
...
class HepatitisData(BaseModel):
    ...
    sex: Optional[str] = Field(
        None, description="Sex of the patient [f, m] ('f'=female, 'm'=male)"
    )
    ALB: Optional[float] = Field(None, description="Amount of albumin in patient's blood")
    ALP: Optional[float] = Field(
        None, description="Amount of alkaline phosphatase in patient's blood"
    )
    ALT: Optional[float] = Field(
        None, description="Amount of alanine transaminase in patient's blood"
    )
    AST: Optional[float] = Field(
        None, description="Amount of aspartate aminotransferase in patient's blood"
    )
    ...
    GGT: Optional[float] = Field(
        None, description="Amount of gamma-glutamyl transferase in patient's blood"
    )
    ...
    def validate_diagnosis(cls, v):
        if v not in ("Healthy", "Hepatitis"):
            raise ValueError(
                "Invalid value for Diagnosis, allowed values are 'Healthy' and 'Hepatitis'"
            )
        return v
