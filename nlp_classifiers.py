...
class HuggingFaceClassifier(BaseClassifier):
    ...
    def preprocess_function(self, examples):
        return self.tokenizer(examples["medical_report"], truncation=True, padding='max_length', max_length=512,
    ...
    def split_datasets(self, df):
        ...
        train_df, val_df = train_test_split(train_df, test_size=0.25, random_state=42,
                                            stratify=train_df["HeartDisease"])
    ...
    def tokenize_datasets(self, train_df, val_df, test_df):
        ...
        tokenized_train_dataset = tokenized_train_dataset.map(lambda examples: {'labels': examples["HeartDisease"]})
        tokenized_val_dataset = tokenized_val_dataset.map(lambda examples: {'labels': examples["HeartDisease"]})
        tokenized_test_dataset = tokenized_test_dataset.map(lambda examples: {'labels': examples["HeartDisease"]})
        return tokenized_train_dataset, tokenized_val_dataset, tokenized_test_dataset

    def fit(self, train_df, val_df, test_df):
        tokenized_train_dataset, tokenized_val_dataset, _ = self.tokenize_datasets(train_df, val_df, test_df)
...
if __name__ == '__main__':
    ...
    def evaluate_and_plot(classifier, test_df, model_name, dataset_name):
        ...
        plot_roc_curve(fpr, tpr, roc_auc, test_df["HeartDisease"].to_list(), model_name, dataset_name)
        return eval_results
    ...
    for model_name, Classifier in heart_disease_classifiers:
        ...
        model_scores.append((model_name, eval_results["accuracy"], eval_results["precision"], eval_results["recall"],
                             eval_results["f1"], eval_results["auc"]))
